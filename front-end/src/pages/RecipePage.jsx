...
import { getRecommendationByDateApi } from '../services/api' // Sesuaikan path jika berbeda
...
export const RecipePage = () => {
  ...
  const { id }    = useParams() // Ambil ID dari URL (e.g. /rekomendasi/resep/417218)

  const [meal, setMeal] = useState(state?.meal || null)
  const [loading, setLoading] = useState(!state?.meal)
  const [error, setError] = useState(false)

  // Fetch data dari API jika state tidak tersedia (misal: user refresh halaman)
  useEffect(() => {
    if (meal) return // Jika sudah ada dari state router, tidak perlu fetch ulang
    ...
    const fetchRecipeFromPlan = async () => {
      ...
      try {
        ...
        // Cari resep yang ID-nya cocok dengan ID di URL
        const found = plan.find(m => String(m.recipe_id) === String(id) || String(m.id) === String(id))

        if (found) {
          // Normalisasi agar propertinya sesuai dengan yang diharapkan oleh UI
          setMeal({
            id: found.recipe_id || found.id,
            name: found.recipe_name || found.name || 'Unknown Recipe',
            emoji: '🍽️',
            image_url: found.image_url || null,
            recommendation_score: found.recommendation_score > 0 ? found.recommendation_score : 0.85,
            ...
            description: found.description || 'A nutritious and balanced meal recommended by your AI plan for sustained energy.',
            nutrition: {
            ...
            recipe: found.recipe || {
              // Fallback karena API di atas belum menyediakan detail ingredients dan steps
              description: 'Recipe details are currently generated by AI. Enjoy your healthy meal!',
              ingredients: ['Follow detailed recipe link or check standard preparations.', 'Salt and pepper to taste.', 'Olive oil for cooking.'],
              steps: ['Prepare and wash all ingredients.', 'Cook according to standard procedures.', 'Serve warm and enjoy!']
            }
          })
        } else {
          setError(true)
      ...
      <>
        ...
        <div className="rp" style={{ display:'flex', alignItems:'center', justifyContent:'center', minHeight:'100vh' }}>
          <div style={{ textAlign:'center' }}>
            <div style={{ fontSize:14, color:'#9CA3AF' }}>Loading recipe...</div>
          </div>
        </div>
      </>
    )
      ...
      <>
        ...
        <div className="rp" style={{ display:'flex', alignItems:'center', justifyContent:'center', minHeight:'100vh' }}>
          <div style={{ textAlign:'center' }}>
            ...
            <button onClick={() => navigate('/recommendations')} style={{ padding:'9px 20px', borderRadius:9, background:'#F97316', border:'none', color:'#fff', fontWeight:600, cursor:'pointer', fontSize:13 }}>
              Back
            </button>
          </div>
        </div>
    ...
    <>
      ...
      <div className="rp">
        <div className="rp-page">
          ...
          {/* Back */}
          <button className="rp-back" onClick={() => navigate('/recommendations')}>
            <Icons.Back /> Back to recommendations
          </button>

          {/* Badges */}
          <div className="rp-badges">
            <span className="rp-badge-match"><Icons.Star /> {((meal.recommendation_score || 0.85) * 100).toFixed(0)}% match</span>
